...
disp('DVS -> compute ITTI MOD saliency map')
...
disp('METTERE LE FISSAZIONI COME NUMERI INTERI')
for f=1:nfix
    %disp('original')
    %fix(f,1)
    %fix(f,2)
    ...
    %creo la matrice binaria
    fixationMap(fixations(f,2),fixations(f,1))=1;
    
end

disp('CONCLUSO e CALCOLA METRICHE IN AUTOMATICO')
